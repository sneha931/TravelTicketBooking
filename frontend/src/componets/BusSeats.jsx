...
const BusSeats = ({ token }) => {
    ...
    const handleBook = async (seatId) => {
        ...
        const seatToBook = seats.find(seat => seat.id === seatId);
        if (!seatToBook) return;

        // Ask for confirmation before booking
        const confirmBooking = window.confirm(
          `Are you sure you want to book Seat ${seatToBook.seat_number}?\n\n` +
          `Bus: ${bus.bus_name}\n` +
          `Route: ${bus.start} → ${bus.destination}\n` +
          `Price: ₹${bus.price}\n\n` +
        ...
        try {
          ...
          // Refresh bus data to get updated available seats count
          const updatedBusResponse = await axios(`https://travelticketbooking.onrender.com/api/buses/${busId}`);
          setBus(updatedBusResponse.data);
                            ...
                            <div>
                                ...
                                <div className="space-y-2">
                                    <p className="flex items-center text-gray-600">
                                        ...
                                        <span className="font-medium">Route:</span> {bus.start} → {bus.destination}
                                    </p>
                                    <p className="flex items-center text-gray-600">
                                        <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <p className="flex items-center text-gray-600">
                                        ...
                                        <span className="font-medium">Bus Number:</span> {bus.bus_number}
                                    </p>
                                    <div className="flex items-center justify-between bg-gray-50 p-2 rounded">
                                        <div className="flex items-center">
                                            ...
                                            <span className="font-medium">Total Seats:</span> {bus.number_seats}
                                        </div>
                                        <div className="flex items-center">
                                            <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path>
                                            </svg>
                                            <span className="font-medium">Available:</span> {bus.available_seats || bus.number_seats}
                                        </div>
                                    </div>
                                </div>
                            </div>
