...
const BusList = ({token}) => {
    ...
    useEffect(() => {
        const fetchBuses = async () => {
            try {
                const response = await axios.get("https://travelticketbooking.onrender.com/api/buses/")
    ...
    const filteredBuses = buses.filter(bus => {
        ...
        const matchesOrigin = filterOrigin ? bus.start.toLowerCase() === filterOrigin.toLowerCase() : true
        const matchesDestination = filterDestination ? bus.destination.toLowerCase() === filterDestination.toLowerCase() : true
        return matchesSearch && matchesOrigin && matchesDestination
    })

    const uniqueOrigins = [...new Set(buses.map(bus => bus.start))]
                            ...
                            <div className="p-6">
                                <div className="flex justify-between items-start">
                                    <div>
                                        ...
                                        <p className="text-gray-600">Bus No: {bus.bus_number}</p>
                                    </div>
                                ...
                                <div className="mt-4">
                                    <div className="flex items-center text-gray-600 mb-2">
                                        <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path>
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                        </svg>
                                        <span>{bus.start} → {bus.destination}</span>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between text-gray-600 bg-gray-50 p-2 rounded">
                                        <div className="flex items-center">
                                            ...
                                            <span>Total: {bus.number_seats}</span>
                                        </div>
                                        <div className="flex items-center">
                                            <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path>
                                            </svg>
                                            <span>Available: {bus.available_seats || bus.number_seats}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="mt-6">
                                    <div className="grid grid-cols-2 gap-3">
                                        <button
                                            ...
                                            className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded transition-colors duration-300 text-sm"
                                        >
                                            View Bus Details
                                        </button>
                                        <button
                                            onClick={() => handleViewSeats(bus.id)}
                                            className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded transition-colors duration-300 text-sm"
                                        >
                                            Book Now
                                        </button>
                                    </div>
